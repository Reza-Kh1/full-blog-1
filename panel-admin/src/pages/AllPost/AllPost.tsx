...
export default function AllPost() {
  ...
  const getData = async () => {
    const data = await fetchApi({ url: `/post/admin-all?page=${page}&status=${status}`, method: "GET" })
    if (data.error) {
      setResponse(null);
      return
    }
    setResponse(data.json.data);
  };
  const deletePost = async (id: number) => {
    const datas = await fetchApi({ url: `/post/${id}`, method: "DELETE" })
            ...
            >
              ...
              <div className="w-4/12">
                <figure className="flex items-center flex-col">
                  {i.imgSrc && (
                    <img src={i.imgSrc} alt="" width={250}
                      height={250}
